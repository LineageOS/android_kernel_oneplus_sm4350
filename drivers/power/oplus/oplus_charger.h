...
#define CONFIG_OPLUS_CALL_MODE_SUPPORT
#endif
...
#elif defined CONFIG_OPLUS_HOLI_CHARGER
#include "../supply/qcom/oplus_battery_holi.h"
...
#define OPCHG_INPUT_CURRENT_LIMIT_CAMERA_MA	900
...
typedef enum {
	LED_SKIN_TEMP_STATUS__NORMAL = 0,		/*<=39c*/
	LED_SKIN_TEMP_STATUS__LITTLE_WARM,		/*>39 && <=41C*/
	LED_SKIN_TEMP_STATUS__WARM,				/*>41 && <=43C*/
	LED_SKIN_TEMP_STATUS__LITTLE_HIGH,		/*>43 && <=45C*/
	LED_SKIN_TEMP_STATUS__HIGH,				/*>45C*/
}
OPLUS_CHG_SKIN_TLED_STATUS;
...
struct oplus_chg_limits {
	...
	int nor_skin_thermal_normal_threshold;			//37C
	int nor_skin_thermal_pre_warm_threshold;		//39C
	int nor_skin_thermal_pre_warm_threshold_antishake;
	int nor_skin_thermal_warm_threshold;			//41C
	int nor_skin_thermal_warm_threshold_antishake;
	int nor_skin_thermal_pre_high_threshold;		//43C
	int nor_skin_thermal_pre_high_threshold_antishake;
	int nor_skin_thermal_high_threshold;			//45C
	int nor_skin_thermal_high_threshold_antishake;
	int input_current_led_ma_skin_normal;
	int input_current_led_ma_skin_warm;
	int input_current_led_ma_skin_med;
	int input_current_led_ma_skin_high;
	int input_current_limit;
	int charge_current_limit;
	int normal_current_cdp_ma;
};
...
/*	struct battery_data battery_main	*/
	...
	struct work_struct load_lcd_work;
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 0))
...
struct oplus_chg_operations {
	...
	int (*get_notify_code)(int notify_code);
#ifdef CONFIG_OPLUS_SHIP_MODE_SUPPORT
	void (*set_shipmode)(int val);
...
static unsigned int is_aging_test = AGING_TEST_STATUS_DEFAULT;
...
unsigned int __attribute__((weak)) set_eng_version(int status)
{
