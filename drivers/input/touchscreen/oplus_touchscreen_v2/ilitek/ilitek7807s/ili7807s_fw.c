				       ...
				       u32 split_len)
{
	...
	if (split_len != 0) {
		for (addr = start, i = 0; addr < end; addr += split_len, i += split_len) {
			...
			if (fix_4_alignment) {
				...
				ILI_INFO("idev->update_buf[5 + 0x%X] = 0x%X\n", split_len - 4,
					 ilits->update_buf[5 + split_len - 4]);
				ILI_INFO("idev->update_buf[5 + 0x%X] = 0x%X\n", split_len - 3,
					 ilits->update_buf[5 + split_len - 3]);
				ILI_INFO("idev->update_buf[5 + 0x%X] = 0x%X\n", split_len - 2,
					 ilits->update_buf[5 + split_len - 2]);
				ILI_INFO("idev->update_buf[5 + 0x%X] = 0x%X\n", split_len - 1,
					 ilits->update_buf[5 + split_len - 1]);
			...
			if (ilits->spi_write_then_read(ilits->spi, ilits->update_buf, split_len + 5,
						       NULL, 0)) {
		...
		if (ilits->spi_write_then_read(ilits->spi, ilits->update_buf, w_len + 5, NULL,
					       0) < 0) {
			ILI_ERR("Failed to write data via SPI in host download (%x)\n", w_len + 5);
			return -EIO;
		}
...
static int ilitek_tddi_fw_iram_upgrade(u8 *pfw, bool mcu)
{
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		if (fbi[i].mode == mode && fbi[i].len != 0) {
			...
			if (ilitek_tddi_fw_iram_program(fbi[i].mem_start, (fw_ptr + fbi[i].start),
							fbi[i].len,
							SPI_UPGRADE_LEN) < 0) {
			...
			if (mode != GESTURE) {
				ilitek_tddi_fw_iram_read(crc_temp, (fbi[i].mem_start + fbi[i].len - crc_len),
							 sizeof(crc_temp));
				iram_crc = crc_temp[0] << 24 | crc_temp[1] << 16 | crc_temp[2] << 8 |
					   crc_temp[3];
			...
			ILI_INFO("%s CRC is %s hex(%x) : dma(%x) : iram(%x), calculation len is 0x%x\n",
				 fbi[i].name, ((crc != dma)
					       || (iram_crc_err)) ? "Invalid !" : "Correct !", crc, dma, iram_crc,
				 fbi[i].len - crc_len);
...
static int ilitek_fw_calc_file_crc(u8 *pfw)
{
	...
	for (i = 0; i < ARRAY_SIZE(fbi); i++) {
		...
		file_crc = pfw[ex_addr - 3] << 24 | pfw[ex_addr - 2] << 16 | pfw[ex_addr - 1] <<
			   8 | pfw[ex_addr];
...
static int ilitek_tddi_fw_update_block_info(u8 *pfw)
{
	...
	fbi[AP].mem_start = (fbi[AP].fix_mem_start != INT_MAX) ? fbi[AP].fix_mem_start :
			    0;
	fbi[DATA].mem_start = (fbi[DATA].fix_mem_start != INT_MAX) ?
			      fbi[DATA].fix_mem_start :
			      DLM_START_ADDRESS;
	fbi[TUNING].mem_start = (fbi[TUNING].fix_mem_start != INT_MAX) ?
				fbi[TUNING].fix_mem_start :
				fbi[DATA].mem_start + fbi[DATA].len;
	fbi[MP].mem_start = (fbi[MP].fix_mem_start != INT_MAX) ? fbi[MP].fix_mem_start :
			    0;
	fbi[GESTURE].mem_start = (fbi[GESTURE].fix_mem_start != INT_MAX) ?
				 fbi[GESTURE].fix_mem_start : 0;
	fbi[TAG].mem_start = (fbi[TAG].fix_mem_start != INT_MAX) ?
			     fbi[TAG].fix_mem_start : 0;
	fbi[PARA_BACKUP].mem_start = (fbi[PARA_BACKUP].fix_mem_start != INT_MAX) ?
				     fbi[PARA_BACKUP].fix_mem_start : 0;
	fbi[DDI].mem_start = (fbi[DDI].fix_mem_start != INT_MAX) ?
			     fbi[DDI].fix_mem_start : 0;
	/* Parsing gesture info form AP code */
	ges_info_addr = (fbi[AP].end + 1 - 60);
	fbi[GESTURE].mem_start = (pfw[ges_info_addr + 7] << 24) +
				 (pfw[ges_info_addr + 6] << 16) +
				 (pfw[ges_info_addr + 5] << 8) + pfw[ges_info_addr + 4];
	ap_end = (pfw[ges_info_addr + 11] << 24) + (pfw[ges_info_addr + 10] << 16) +
		 (pfw[ges_info_addr + 9] << 8) + pfw[ges_info_addr + 8];
	...
	ipio_memcpy(ilits->fw_info, (pfw + fw_info_addr), sizeof(ilits->fw_info),
		    sizeof(ilits->fw_info));
	/* copy fw mp ver */
	fw_mp_ver_addr = fbi[MP].end - INFO_MP_HEX_ADDR;
	ILI_INFO("Parsing hex mp ver addr = 0x%x\n", fw_mp_ver_addr);
	ipio_memcpy(ilits->fw_mp_ver, pfw + fw_mp_ver_addr, sizeof(ilits->fw_mp_ver),
	...
	ipio_memcpy(&ilits->rib, ilits->fw_info, sizeof(ilits->rib),
		    sizeof(ilits->rib));
...
static int ilitek_tddi_fw_ili_convert(u8 *pfw)
{
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM - 1; i++) {
		/* B0 tag */
		b0_addr = (CTPM_FW[4 + i * 4] << 16) | (CTPM_FW[5 + i * 4] << 8) |
			  (CTPM_FW[6 + i * 4]);
		...
		if (((blk_map >> i) & 0x01) == 0x01) {
			fbi[num].start = (CTPM_FW[132 + i * 6] << 16) | (CTPM_FW[133 + i * 6] << 8) |
					 CTPM_FW[134 + i * 6];
			fbi[num].end = (CTPM_FW[135 + i * 6] << 16) | (CTPM_FW[136 + i * 6] << 8) |
				       CTPM_FW[137 + i * 6];
...
static int ilitek_tdd_fw_hex_open(u8 op, u8 *pfw)
{
	...
	if (ilitek_tddi_fw_hex_convert((u8 *)ilits->tp_fw.data, ilits->tp_fw.size,
				       pfw) < 0) {
...
out:
	ili_vfree((void **) & (ilits->tp_fw.data));
	return ret;
}
...
{
	int i, ret = 0, retry = 3;
	static bool set_esd_check = true;

	if (ilits->oplus_fw_update || ilits->force_fw_update || ERR_ALLOC_MEM(pfw)) {
	...
	ILI_INFO("Upgrade pass\n");

	/*Waiting for fw ready sending first cmd*/
	mdelay(50);
out:
	ilits->sleep_type = NOT_SLEEP_MODE;
	ili_ic_get_core_ver();
	...
	if (ilits->debug_mode_sta && ilits->actual_tp_mode != P5_X_FW_GESTURE_MODE) {
		if (ilits->ts->apk_op && ilits->ts->apk_op->apk_debug_set) {
			ilits->ts->apk_op->apk_debug_set((void *)ilits, true);
		}
	}
