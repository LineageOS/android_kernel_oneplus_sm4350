...
static int ilitek_debug_node_buff_control(bool open)
{
	...
	if (open) {
		...
		for (i = 0; i < TR_BUF_LIST_SIZE; i++) {
			...
			ili_kfree((void **)&ilits->dbl[i].data);
...
static int debug_mode_get_data(struct file_buffer *file, u8 type,
			       u32 frame_count)
{
	int ret;
	u8 cmd[2] = { 0 }, row, col;
		...
		loff_t *pos)
{
	...
	len = snprintf(g_user_buf + size, PAGE_SIZE - len,
		       "======== Deltadata ========\n");
	ILI_INFO("======== Deltadata ========\n");
		...
		loff_t *pos)
{
	...
	if (ERR_ALLOC_MEM(tmpbuf)) {
		...
		send_data_len += snprintf(tmpbufback + send_data_len, sizeof(tmpbufback),
	...
	if (ilits->dbl[ilits->odi].mark) {
		...
		for (i = 0; i < need_read_data_len; i++) {
			send_data_len += snprintf(tmpbuf + send_data_len, sizeof(tmpbufback), "%02X",
						  ilits->dbl[ilits->odi].data[i]);
...
static ssize_t ilitek_node_ver_info_read(struct file *filp, char __user *buff,
		size_t size,
		loff_t *pos)
{
	u32 len = 0;
...
static ssize_t ilitek_node_change_list_read(struct file *filp,
		char __user *buff, size_t size,
		loff_t *pos)
{
	u32 len = 0;
		...
		loff_t *pos)
{
	...
	memset(g_user_buf, 0, USER_STR_BUFF * sizeof(unsigned char));
	ili_debug_en = !ili_debug_en;
	ILI_INFO(" %s debug level = %x\n", ili_debug_en ? "Enable" : "Disable",
		 ili_debug_en);
...
int ili_get_tp_recore_ctrl(int data)
{
	...
	case DATA_RECORD:
		...
		ret = ili_get_tp_recore_data(NULL, ilits->xch_num * ilits->ych_num * 2);
...
int ili_get_tp_recore_data(u16 *out_buf, u32 out_len)
{
	u8 buf[8] = {0};
	u8 record_case = 0;
	s8 index;
	...
	ILI_INFO("addr = 0x%x, len = %d, lndex = %d, frame_cnt = %d, record_case = 0x%x\n",
		 addr, len, index, frame_cnt, record_case);
	ili_dump_data(buf, 8, sizeof(buf), 0, "all record bytes");

	if (len > 4096) {
		ILI_ERR("ilitek_tddi_get_tp_recore_data len is %d.\n", len);
		return -1;
	}
	...
	/*    ili_ice_mode_read((addr + i), &ptr[j], sizeof(u32));*/
	/*}*/
	buf[0] = 0x25;
	...
	if (frame_cnt > 0 && index >= 0 && index < 3) {
		...
		if (out_buf) {
			if (out_len > frame_len * 2) {
				ILI_INFO("out_len is %d, frame_len is %d\n", out_len, frame_len);
				out_len = frame_len * 2;
			}
				       ...
				       loff_t *pos)
{
	...
	if (strncmp(cmd, "hwreset", strlen(cmd)) == 0) {
		...
		ilits->esd_check_enabled = true;
		ILI_INFO("esd_check_enabled flag= %d\n", ilits->esd_check_enabled);

	} else if (strncmp(cmd, "disablewqesd", strlen(cmd)) == 0) {
		ilits->esd_check_enabled = false;
		ILI_INFO("esd_check_enabled flag= %d\n", ilits->esd_check_enabled);
				     ...
				     unsigned long arg)
{
	...
	case ILITEK_COMPAT_IOCTL_I2C_WRITE_DATA:
		...
		ret = filp->f_op->unlocked_ioctl(filp, ILITEK_IOCTL_I2C_WRITE_DATA,
						 (unsigned long)compat_ptr(arg));
		return ret;

	case ILITEK_COMPAT_IOCTL_I2C_READ_DATA:
		ILI_DBG("compat_ioctl: convert i2c/spi read\n");
		ret = filp->f_op->unlocked_ioctl(filp, ILITEK_IOCTL_I2C_READ_DATA,
						 (unsigned long)compat_ptr(arg));
		return ret;
	...
	case ILITEK_COMPAT_IOCTL_TP_INFO:
		...
		ret = filp->f_op->unlocked_ioctl(filp, ILITEK_IOCTL_TP_INFO,
						 (unsigned long)compat_ptr(arg));
		return ret;

	case ILITEK_COMPAT_IOCTL_WRAPPER_RW:
		ILI_DBG("compat_ioctl: convert wrapper\n");
		ret = filp->f_op->unlocked_ioctl(filp, ILITEK_IOCTL_WRAPPER_RW,
						 (unsigned long)compat_ptr(arg));
		return ret;

	case ILITEK_COMPAT_IOCTL_DDI_WRITE:
		ILI_DBG("compat_ioctl: convert ddi write\n");
		ret = filp->f_op->unlocked_ioctl(filp, ILITEK_IOCTL_DDI_WRITE,
						 (unsigned long)compat_ptr(arg));
		return ret;

	case ILITEK_COMPAT_IOCTL_DDI_READ:
		ILI_DBG("compat_ioctl: convert ddi read\n");
		ret = filp->f_op->unlocked_ioctl(filp, ILITEK_IOCTL_DDI_READ,
						 (unsigned long)compat_ptr(arg));
		return ret;
			      ...
			      unsigned long arg)
{
	...
	case ILITEK_IOCTL_TP_DEBUG_LEVEL:
		...
		ili_debug_en = !ili_debug_en;
		ILI_DBG("ili_debug_en = %d", ili_debug_en);
		break;
	...
	case ILITEK_IOCTL_TP_DRV_VER:
		...
		length = snprintf(szBuf, USER_STR_BUFF * sizeof(unsigned char), "%s",
				  DRIVER_VERSION);
	...
	case ILITEK_IOCTL_TP_DUMP_FLASH:
		ILI_INFO("ioctl: dump flash data. spi interface do nothing\n");
		ret = 0;
	...
	ili_kfree((void **)&szBuf);
	ili_kfree((void **)&wrap_rbuf);
out:
	mutex_unlock(&ilits->touch_mutex);
	return ret;
}
...
void ili_node_init(void)
{
	...
	for (; i < ARRAY_SIZE(iliproc); i++) {
		iliproc[i].node = proc_create(iliproc[i].name, 0644, proc_dir_ilitek,
					      iliproc[i].fops);
