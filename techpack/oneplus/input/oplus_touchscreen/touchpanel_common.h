...
#define S_GESTURE            14  // S
#define SINGLE_TAP           15
...
struct monitor_data {
	...
	struct list_head        short_swipe_zone_list;
	...
	struct points_record    jumping_points;
	struct points_record    stuck_points;
	struct points_record    lanscape_stuck_points;
	struct swipes_record    long_swipes;
	struct swipes_record    broken_swipes;
...
struct touchpanel_data {
    ...
    char input_name[TP_NAME_SIZE_MAX];
    char input_ps_name[TP_NAME_SIZE_MAX];
    char input_kpd_name[TP_NAME_SIZE_MAX];
...
struct oplus_touchpanel_operations {
    ...
    int (*specific_resume_operate) (void *chip_data, struct specific_resume_data *p_resume_data); /*some ic need specific opearation in resuming*/
...
void input_report_key_oplus(struct touchpanel_data *ts, unsigned int code, int value);
...
extern bool __init tp_judge_ic_match(char *tp_ic_name);
