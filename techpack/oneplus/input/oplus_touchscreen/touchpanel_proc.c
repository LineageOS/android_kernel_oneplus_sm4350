...
__attribute__((weak)) int register_device_proc(char *name, char *version, char *manufacture)
{
    return 0;
}
#endif
uint8_t DouTap_enable = 0; // double tap
EXPORT_SYMBOL(DouTap_enable);
uint8_t UpVee_enable  = 0; // V
EXPORT_SYMBOL(UpVee_enable);
uint8_t LeftVee_enable = 0; // >
EXPORT_SYMBOL(LeftVee_enable);
uint8_t RightVee_enable = 0; // <
EXPORT_SYMBOL(RightVee_enable);
uint8_t Circle_enable = 0; // O
EXPORT_SYMBOL(Circle_enable);
uint8_t DouSwip_enable = 0;  // ||
EXPORT_SYMBOL(DouSwip_enable);
uint8_t Mgestrue_enable = 0; // M
EXPORT_SYMBOL(Mgestrue_enable);
uint8_t Wgestrue_enable = 0; // W
EXPORT_SYMBOL(Wgestrue_enable);
uint8_t Sgestrue_enable = 0; // S
EXPORT_SYMBOL(Sgestrue_enable);
uint8_t SingleTap_enable = 0; // single tap
EXPORT_SYMBOL(SingleTap_enable);
uint8_t Enable_gesture = 0;
EXPORT_SYMBOL(Enable_gesture);
 ...
 */
static ssize_t proc_index_control_read(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
    uint8_t ret = 0;
 ...
 * tp_index: current pointer tp touchpanel_data;
 */
static ssize_t proc_index_control_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    int tmp = 0;
    char buf[4] = {0};
 ...
 */
static ssize_t proc_debug_level_read(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
    uint8_t ret = 0;
    char page[PAGESIZE] = {0};
 ...
 * tp_debug:2, LEVEL_DEBUG;
 */
static ssize_t proc_debug_level_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
...
static ssize_t proc_coordinate_read(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    ret = snprintf(page, PAGESIZE - 1, "%d,%d:%d,%d:%d,%d:%d,%d:%d,%d:%d,%d:%d,%d\n", ts->gesture.gesture_type,
...
/*game_switch_enable - For Tengxun game Hepingjingying
 * Input:
...
static ssize_t proc_game_switch_read(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data *ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_gesture_switch_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	TPD_INFO("%s: gesture_switch value= %d\n", __func__, value);
 ...
 * irq gpio state;
 */
static ssize_t proc_get_irq_depth_read(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data *ts = PDE_DATA(file_inode(file));
 ...
 */
static ssize_t proc_irq_status_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    int value = 0;
    char buf[4] = {0};
    struct touchpanel_data *ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_noise_modetest_read(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
    ssize_t ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data *ts = PDE_DATA(file_inode(file));
 ...
 */
static ssize_t proc_fw_update_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    struct touchpanel_data *ts = PDE_DATA(file_inode(file));
    int val = 0;
    int ret = 0;
...
#endif
    {
        TP_INFO(ts->tp_index, "boot mode is MSM_BOOT_MODE__CHARGE,not need update tp firmware\n");
        return count;
    }
 ...
 */
static ssize_t proc_register_info_read(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
    int ret = 0;
    int i = 0;
...
static ssize_t proc_register_info_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    TP_INFO(ts->tp_index, "ts->reg_info.reg_addr = 0x%x, ts->reg_info.reg_lenght = %d\n", ts->reg_info.reg_addr, ts->reg_info.reg_length);
...
static ssize_t proc_incell_panel_info_read(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
    uint8_t ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data *ts = PDE_DATA(file_inode(file));
 ...
 * fd_enable:1, disable face detect;
 */
static ssize_t proc_fd_enable_read(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data *ts = PDE_DATA(file_inode(file));
 ...
 */
static ssize_t proc_event_num_read(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
    int ret = 0;
 ...
 * touch_count:1-F;
 */
static ssize_t proc_fd_touch_read(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data *ts = PDE_DATA(file_inode(file));
 ...
 * fp_enable:1, disable;
 */
static ssize_t proc_fp_enable_read(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data *ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_black_screen_test_read(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
    int ret = 0;
...
static ssize_t proc_smooth_level_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    if (!ts->is_suspended) {
        ...
        ts->ts_ops->smooth_lv_set(ts->chip_data, value);
...
static ssize_t proc_wireless_charge_detect_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    if (ts->wireless_charger_support && (ts->is_wireless_checked != wireless_state)) {
...
static ssize_t proc_headset_detect_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    int headset_state = 0 ;
    char buf[4] = {0};
    struct touchpanel_data *ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_headset_detect_read(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data *ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_aging_test_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    struct touchpanel_data *ts = PDE_DATA(file_inode(file));
    int val = 0;
    char buf[4] = {0};
...
static int tp_main_register_read_func(struct seq_file *s, void *v)
{
    ...
    if (ts->health_monitor_support && tp_debug == 2) {
        if (monitor_data->fw_version) {
            ...
            strncpy(monitor_data->fw_version, ts->panel_data.manufacture_info.version, strlen(ts->panel_data.manufacture_info.version));
        }
...
static ssize_t proc_freq_hop_read(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
    int ret = 0;
    char page[PAGESIZE] = {0};
    struct touchpanel_data *ts = PDE_DATA(file_inode(file));
...
static ssize_t proc_chip_info_read(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
	int ret = 0;
	char page[50] = {0};
	struct touchpanel_data *ts = PDE_DATA(file_inode(file));

	if (!ts || ts->panel_data.chip_name == NULL) {
		sprintf(page, "%d\n", -1);//no support
	} else {
		if (ts->panel_data.chip_num == 1)
			sprintf(page, "%s_%02d\n", ts->panel_data.chip_name[0], ts->chip_version);//support
		else
			sprintf(page, "%d\n", -1);//no support
	}
	ret = simple_read_from_buffer(user_buf, count, ppos, page, strlen(page));
...
static const struct file_operations proc_chip_info_fops = {
	.read  = proc_chip_info_read,
	.open  = simple_open,
	.owner = THIS_MODULE,
};
...
typedef struct {
    ...
    void *data;
    bool isCreated;//proc node is creater or not
    bool isSupport;//feature is supported or not
} tp_proc_node;
...
static int init_debug_info_proc(struct touchpanel_data *ts)
{
    ...
    for (i = 0; i < ARRAY_SIZE(proc_debug_node); i++) {
        if (proc_debug_node[i].isSupport) {
            proc_debug_node[i].node = proc_create_data(proc_debug_node[i].name, proc_debug_node[i].mode,
                                      prEntry_debug_info, proc_debug_node[i].fops, proc_debug_node[i].data);

            if (proc_debug_node[i].node == NULL) {
                proc_debug_node[i].isCreated = false;
                TP_INFO(ts->tp_index, "%s: Couldn't create proc/debug_info/%s\n", __func__, proc_debug_node[i].name);
                ret = -ENODEV;
            } else {
                proc_debug_node[i].isCreated = true;
            }
        }
    }
...
int init_touchpanel_proc(struct touchpanel_data *ts)
{
    ...
    tp_proc_node tp_proc_node[] = {
        ...
        {
            "baseline_test_result", 0666, NULL, &tp_auto_test_result_fops, ts, false, true
        },
        {
        ...
        {
            "smooth_level_support", 0666, NULL, &proc_smooth_level_fops, ts, false,
	...
	{"tpedge_limit_enable",0666, NULL, &proc_limit_switch_fops, ts, false, true},
...
#endif // end of CONFIG_OPLUS_TP_APK
	{"chip_info", 0666, NULL, &proc_chip_info_fops, ts, false, true},
	{"gesture_switch", 0666, NULL, &proc_gesture_switch_fops, ts, false, ts->black_gesture_support},
    };
    ...
    if(ts->fw_update_app_support) {
        ...
        register_device_proc(name, ts->panel_data.manufacture_info.version, ts->panel_data.manufacture_info.manufacture);
    }
#endif
    ...
    for (i = 0; i < ARRAY_SIZE(tp_proc_node); i++) {
        if (tp_proc_node[i].isSupport) {
            tp_proc_node[i].node = proc_create_data(tp_proc_node[i].name, tp_proc_node[i].mode,
                                                    prEntry_tp, tp_proc_node[i].fops, tp_proc_node[i].data);

            if (tp_proc_node[i].node == NULL) {
                tp_proc_node[i].isCreated = false;
                TP_INFO(ts->tp_index, "%s: Couldn't create proc/debug_info/%s\n", __func__, tp_proc_node[i].name);
                ret = -ENODEV;
            } else {
