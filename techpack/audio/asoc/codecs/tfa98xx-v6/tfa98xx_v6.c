...
//Chunyu.xie@MULTIMEDIA.AUDIODRIVER.FEATURE, 2020/07/22, use proc fs to replace debug fs
...
/* Zhao.Pan@MULTIMEDIA.AUDIODRIVER.HeadsetDet, 2021/06/03, Add for smartpa err feedback. */
...
/*Jianfeng.Qiu@PSW.MM.AudioDriver.SmartPA, 2019/07/20,
 *Add for tfa98xx vi feedback.
...
#ifdef OPLUS_ARCH_EXTENDS
/*Jianfeng.Qiu@PSW.MM.AudioDriver.SmartPA, 2017/01/03, Add for get spk revsion*/
...
#ifdef OPLUS_ARCH_EXTENDS
/*Jianfeng.Qiu@PSW.MM.AudioDriver.FTM.954616, 2017/01/03, Add for spk ftm test*/
...
#ifndef OPLUS_ARCH_EXTENDS
/*Wei.Hua@MULTIMEDIA.AUDIODRIVER.SMARTPA, 2021/01/06, compatible with profs*/
/* OTC reporting
...
#endif
#ifndef OPLUS_ARCH_EXTENDS
/*Wei.Hua@MULTIMEDIA.AUDIODRIVER.SMARTPA, 2020/09/10, Add for calibrated status*/
				     ...
				     loff_t *ppos)
{
	...
	if (tfa98xx->tfa->is_probus_device) {
		...
		if (ret) {
			...
			#ifndef OPLUS_ARCH_EXTENDS
			/*Jianfeng.Qiu@PSW.MM.AudioDriver.SmartPA, 2018/07/23, Delete for speaker resistance*/
			goto r_c_err;
			#endif /* OPLUS_ARCH_EXTENDS */
		}
...
//rainz.zhang@MULTIMEDIA.AUDIODRIVER.FEATURE, 2020/10/22, Add for SMT selfcheck
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	#ifndef OPLUS_ARCH_EXTENDS
	/*Jianfeng.Qiu@PSW.MM.AudioDriver.SmartPA, 2018/06/18, Modify for I2C/I2S
                               ...
                               struct snd_ctl_elem_value *ucontrol)
{
	...
	#ifdef OPLUS_FEATURE_SPEAKER_MUTE
	/*Juqiang.Geng@PSW.MM.AudioDriver.PA, 2020/03/30, Add for spk mute ctrl*/
...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
	...
	#ifdef OPLUS_ARCH_EXTENDS
	/* Zhao.Pan@MULTIMEDIA.AUDIODRIVER.SMARTPA, 2021/07/18, Add for check out of bounds */
...
#ifndef OPLUS_ARCH_EXTENDS
/* Jianfeng.Qiu@PSW.MM.AudioDriver.Machine, 2018/05/14,
...
tfa98xx_write_dsp(struct tfa_device *tfa,  int num_bytes, const char *command_buffer)
{
	...
	uint8_t *buffer = NULL;
	#ifdef OPLUS_ARCH_EXTENDS
	/* Zhao.Pan@MULTIMEDIA.AUDIODRIVER.SMARTPA, 2021/12/07, add return error */
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
	#endif
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	*/
	...
	#ifdef OPLUS_ARCH_EXTENDS
	/* xiang.fei@MM.AudioDriver.Codec, 2018/03/12, Add for noise in reboot */
...
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
	...
	bool sync= false;
	#ifdef OPLUS_ARCH_EXTENDS
	/* xiang.fei@MM.AudioDriver.Codec, 2018/03/12, add for calibrate */
	int value = 0;
	#endif /* OPLUS_ARCH_EXTENDS */
	...
	if (tfa98xx->init_count < TF98XX_MAX_DSP_START_TRY_COUNT) {
		...
		ret = tfa98xx_tfa_start(tfa98xx, tfa98xx->profile, tfa98xx->vstep);
		#ifdef OPLUS_ARCH_EXTENDS
		/*Jianfeng.Qiu@PSW.MM.AudioDriver.FTM.1447460, 2018/07/04,
	...
	if (sync) {
		...
		if (do_sync) {
			...
			list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
				mutex_lock(&tfa98xx->dsp_lock);
				#ifndef OPLUS_ARCH_EXTENDS
				/*Jianfeng.Qiu@PSW.MM.AudioDriver.SmartPA, 2018/07/23, Modify for calibrate*/
				tfa_dev_set_state(tfa98xx->tfa, TFA_STATE_UNMUTE);
				#else
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		...
		/* Start DSP */
		#ifndef OPLUS_ARCH_EXTENDS
		/*Jianfeng.Qiu@PSW.MM.AudioDriver.SmartPA.1234963, 2018/05/02, Modify for no sound*/
...
static struct snd_soc_dai_driver tfa98xx_dai[] = {
	{
		...
		.ops = &tfa98xx_dai_ops,
#ifndef OPLUS_ARCH_EXTENDS
/* xiang.fei@MM.AudioDriver.Codec, 2018/03/12,
			     ...
			     const struct i2c_device_id *id)
{
	...
	int ret;
	#ifdef OPLUS_ARCH_EXTENDS
	/* xiang.fei@MM.AudioDriver.Codec, 2018/03/12, Modify for chip id */
	int i = 0;
	int retries = 5;
	#endif /* OPLUS_ARCH_EXTENDS */
...
/*Zhao.Pan@MULTIMEDIA.AUDIODRIVER.HeadsetDet, 2021/06/03, Add for smartpa err feedback*/
	...
	if (!tfa98xx_proc_node) {
		pr_info("%s: create proc '%s'!\n", __func__, TFA98XX_DEBUG_FS_NAME);
		tfa98xx_proc_node = proc_create_data(TFA98XX_DEBUG_FS_NAME,
...
static int tfa98xx_i2c_remove(struct i2c_client *i2c)
{
	...
	#else /* CONFIG_DEBUG_FS */
	if (tfa98xx_proc_node) {
		pr_info("%s: remove tfa98xx_proc_node\n", __func__);
