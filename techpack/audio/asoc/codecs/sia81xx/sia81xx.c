...
static sia81xx_dev_t *g_sia81xx = NULL;
...
{	
	...
	mdelay(2);	/* wait for owi reset, longer than 1ms */
	...
	struct sia81xx_dev_s *sia81xx)
{
	...
	if (is_chip_type_supported(sia81xx->chip_type)) {
		...
		if (0 == sia81xx->disable_pin) {
			...
			mdelay(40);	/* for sia8109 gain rising. */
		}
	...
	if (sia81xx->en_dyn_ud_vdd)	// timer task is only used for VDD updating currently
        ...
        struct snd_ctl_elem_value *ucontrol)
{
	...
	sia81xx_dev_t *sia81xx = snd_soc_codec_get_drvdata(codec);
        ...
        char val;
        const char voltage[] = {0xCC,0xC8,0xC6,0xC0};
        int vol_length = 4;
        int i;
        ...
        struct snd_ctl_elem_value *ucontrol)
{
	...
	sia81xx_dev_t *sia81xx = snd_soc_codec_get_drvdata(codec);
        ...
        if (index > 3 || index < 0) {
            pr_err("[error][%s] %s: invalid param.", LOG_FLAG, __func__);
            index = 0;
        }
...
int soc_check_sia81xx_status()
{
	...
	if (sia81xx == NULL) {
		pr_err("%s sia81xx status is error", __func__);
		return 0;
	}
...
EXPORT_SYMBOL(soc_check_sia81xx_status);
#endif
...
static ssize_t sia81xx_get_registers(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
   sia81xx_dev_t *sia81xx = dev_get_drvdata(dev);
   ssize_t len = 0;
   unsigned int i = 0;
   unsigned char reg_val[0x64];

   pr_info("%s enter, sia8109", __func__);
   sia81xx_regmap_read(
		sia81xx->regmap, 0x00, 0x0D + 1, reg_val);
   for (i = 0; i < 0x0D; i++) {
...
static ssize_t sia81xx_set_registers(struct device *dev,
				 ...
				 size_t len)
{
  ...
  if (sscanf(buf, "%x %x", &reg, &value) == 2)
	  sia81xx_regmap_write(sia81xx->regmap, reg, 1, &value);
  return len;
}

static DEVICE_ATTR(registers, S_IWUSR | S_IRUGO, sia81xx_get_registers, sia81xx_set_registers);

static struct attribute *sia81xx_attributes[] = {
	&dev_attr_registers.attr,
	NULL
};

static struct attribute_group sia81xx_attribute_group = {
	.attrs = sia81xx_attributes
};
	...
	const struct i2c_device_id *id)
{
	...
	/* add  attribute */
	ret = sysfs_create_group(&client->dev.kobj, &sia81xx_attribute_group);
	if (ret < 0) {
		pr_err("[  err][%s] %s: failed to create sysfs nodes\n", LOG_FLAG, __func__);
	}
