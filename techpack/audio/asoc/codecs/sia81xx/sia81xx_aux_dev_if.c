	...
	u32 conf_num)
{
	...
	for (i = 0; i < dev_num; i++) {
		...
		aux_dev[i].dlc.name = "soc:sia81xx@L"; //change name not match.
		aux_dev[i].dlc.dai_name = NULL;
		aux_dev[i].dlc.of_node = dev_of_node;
#else
		aux_dev[i].name = "soc:sia81xx@L"; // (const char *)aux_dev_name;
		aux_dev[i].codec_name = NULL;
		aux_dev[i].codec_of_node = dev_of_node;
#endif
		aux_dev[i].init = NULL;

		codec_conf[i].dev_name = "soc:sia81xx@L";//(const char *)aux_dev_name;
